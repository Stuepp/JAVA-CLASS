		...
		while(ok == 0) {
			...
			if(zoo.getSoAquarios().get(num).espacoDisponivel() >= animal.calculaEspacoOcupado()) {
				try {
					...
					zoo.alocarAnimal(animal, zoo.getSoViveiros().get(num));
				} catch (EspacoIndisponivelException e) {
					System.err.println(e.getMessage());
				}/* catch (TemperaturaInadequadaException e) { teria que criar throws em alocar animal
					System.err.println(e.getMessage());
		...
		while(ok == 0) {
			...
			if(zoo.getSoAquarios().get(num).espacoDisponivel() >= peixe.EspacoOcupado()) {
				try {
					...
					zoo.alocarAnimal(peixe, zoo.getSoAquarios().get(num));
				} catch (EspacoIndisponivelException e) {
					System.err.println(e.getMessage());
				}/* catch (TemperaturaInadequadaException e) {  teria que criar throws em alocar animal
					System.err.println(e.getMessage());
